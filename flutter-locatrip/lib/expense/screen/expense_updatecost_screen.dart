  ...
  Future<void> _fetchExpenseDetails() async {
    try {
      ...
      setState(() {
        ...
        for (var user in [...paidByUsers, ...participants]) {
          final existing = mergedParticipants.firstWhere(
                (u) => u['id'] == user['id'],
            orElse: () => {},
          );

          if (existing == null) {
            mergedParticipants.add({
              'id': user['id'],
              'nickname': user['nickname'],
              'isPaid': user['isPaid'] ?? false, // 기본값 설정
              'isChecked': user['isChecked'] ?? false, // 기본값 설정
            });
          } else {
            existing['isPaid'] = existing['isPaid'] || (user['isPaid'] ?? false);
            existing['isChecked'] = existing['isChecked'] || (user['isChecked'] ?? false);
          }
        }
